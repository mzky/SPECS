...
%if %{static_libcrypto}
perl -pi -e "s|-lcrypto|/usr/local/libressl/lib/libcrypto.a|g" Makefile
%endif
...
install -d -m755 $RPM_BUILD_ROOT/etc/bash_completion.d
...
%attr(0644,root,root) %config(noreplace) /etc/bash_completion.d/ssh-keygen-bash-completion.sh
%attr(0644,root,root) %config(noreplace) /etc/pam.d/ssh-keycat
%attr(0755,root,root) %config(noreplace) /usr/bin/ssh-copy-id
%attr(0644,root,root) %config(noreplace) /usr/lib/systemd/user/ssh-agent.service
